    ...
    def test(self):
        ...
        if flip_seq:
            ...
            with torch.no_grad():
                ...
                #with open(counter_file, 'a+') as f:  # a+
                    #fcntl.flock(f, fcntl.LOCK_EX)  #         
                    #f.seek(0)
                    #content = f.read().strip()
                    #current_idx = int(content) + 1 if content else 1        
                    # 
                    #torch.save(fs, f'./fspt/{current_idx}_fs_tensor.pt')
                    #torch.save(fc, f'./fcpt/{current_idx}_fc_tensor.pt')
                ...
                #modelx=self.net_g
                #print("Model Summary:")
                #summary(modelx, input_size=(1, 4,3, 2160, 3840))
                #flops, params = profile(modelx, inputs=(self.lq,))
                #print(f"$$$FLOPs: {flops / 1e9:.2f} G")  # 转换为 GFLOPs
                #print(f"$$$Params: {params / 1e6:.4f} M")  # 转换为百万参数
